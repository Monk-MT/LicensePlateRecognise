...
#define VERTICAL 0
	...
	bool verifyRotatedPlateSizes(RotatedRect mr, bool showDebug) {
		...
		float min = float(width_min * width_min / aspect_max);
		float max = float(width_max * width_max / aspect_min);
...
#pragma omp parallel for
	...
	Mat adaptive_image_from_points(const std::vector<Point> &points, const Rect &rect, const Size &size,
		const Scalar &backgroundColor, const Scalar &forgroundColor, bool gray) {
		int expendHeight = 0;
		int expendWidth = 0;
