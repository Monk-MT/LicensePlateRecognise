...
#include "LicensePlateRecognise/train/annCh_train.h"
...
#include "LicensePlateRecognise/train/img_processing.h"
...
namespace LicensePlateRecognise {
  ...
  if (files.size() == 0) {
    ...
    fprintf(stdout, "You should create a folder named \"tmp\" in LicensePlateRecognise main folder.\n");
...
void AnnChTrain::trainVal(size_t number_for_count) {
  ...
  for (int i = 0; i < classNumber; ++i) { 
    ...
    for (auto file : chars_files) {
      ...
      auto img = cv::imread(file, IMREAD_GRAYSCALE); 
      Mat img_resize;
