#ifndef LPR_UTIL_UTIL_H_
#define LPR_UTIL_UTIL_H_
  ...
  static unsigned int levenshtein_distance(const T &s1, const T &s2) {
    ...
    for (unsigned int i = 0; i < len1; i++) {
      ...
      for (unsigned int j = 0; j < len2; j++)
        col[j + 1] = LicensePlateRecognise::Utils::min(
            LicensePlateRecognise::Utils::min(prevCol[1 + j] + 1, col[j] + 1),
